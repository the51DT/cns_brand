    ...
    if (pageCateTitle) {
      ...
      data.IaList.some((item) => {
        const matchedItem = item.Level2.find(
          (subItem) => subItem.fileName.includes(currentPageName), // 현재 페이지 경로와 매칭
        );
        if (matchedItem) {
...
async function lnbMenuGuide() {
  fetch('/ia.json')
    ...
    .then((data) => {
      ...
      const filteredData = data.IaList.filter((level1Item) => [1, 2, 3].includes(level1Item.id));
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (guideNavyBtn) {
    guideNavyBtn.addEventListener('click', () => {
      guideNavy.classList.contains('open')
        ? setCls(guideNavy, 'open', 'remove')
        : setCls(guideNavy, 'open');
    });
  }
